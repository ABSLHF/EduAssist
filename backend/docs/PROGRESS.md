...
- 30题检索评测（`context_mode=retrieve`, `hit_threshold=0.2`）：
  ...
  - 2026-02-20 03:10:03：RAG `13/30 (0.4333)`，QA抽取 `3/30 (0.1000)`。
  - 2026-02-20 03:16:47：RAG `17/30 (0.5667)`，QA抽取 `1/30 (0.0333)`。
- 对照结论：
  - `gold` 模式下 QA抽取可达 `30/30`，说明抽取模型能力可用。
  - `retrieve` 模式从 `0.3333` 提升到 `0.5667`，主链路瓶颈仍是检索召回与资料覆盖面。
- 阶段判断：
  - “教师上传资料 -> 检索答疑”主链路已形成可演示闭环。
  - 当前已超过阶段目标 `0.40+`，进入“答辩材料固化 + 剩余低分题精修”阶段。
